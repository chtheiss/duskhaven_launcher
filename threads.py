...
class BackgroundTask(QThread):
    ...
    def run(self):
        ...
        download_path = self.temp_dest_path or self.dest_path
        ...
        with requests.get(self.url, headers=headers, stream=True) as response:
            ...
            with open(download_path, mode) as file:
                for chunk in response.iter_content(chunk_size=8192):
                    ...
                    if chunk:
                        file.write(chunk)
                        file_stats = self.temp_dest_path.stat()
        ...
        if self.temp_dest_path is not None:
            shutil.move(self.temp_dest_path, self.dest_path)
